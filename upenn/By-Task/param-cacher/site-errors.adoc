...
. service/record-types/transcript/searches/GenesByRNASeqUserDataset/ +
  Dataset based searches should not be run by param cacher.  Guest user has no datasets.
...
.Error
----
Caused by: org.gusdb.fgputil.cache.ValueProductionException: org.gusdb.wdk.model.query.param.WdkEmptyEnumListException: No item returned by the query [GeneVQ.GenericPercentileSamples] of FlatVocabParam [geneParams.samples_percentile_generic].
...
Caused by: org.gusdb.wdk.model.query.param.WdkEmptyEnumListException: No item returned by the query [GeneVQ.GenericPercentileSamples] of FlatVocabParam [geneParams.samples_percentile_generic].
